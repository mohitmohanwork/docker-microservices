...
namespace Blaash.Gaming.Infrastructure.Database.Mongo
{
    public class MongoDBKeyValueService : ZNxt.Net.Core.Interfaces.IKeyValueStorage
    {
        ...
        public bool Delete(string bucket, string key)
        {
            ...
            return _dBService.Delete(collection, new ZNxt.Net.Core.Model.RawQuery("{'key':'" + key + "'}")) == 1;
        }
        ...
        {
            var collection = GetCollection(bucket);
            var data = _dBService.Get(collection, new ZNxt.Net.Core.Model.RawQuery("{'"+ CommonConst.CommonField.KEY+"':'" + key + "'}"));
            if (data.Count != 0)
            {
        ...
        {
            var collection = GetCollection(bucket);
            var data = _dBService.Get(collection, new ZNxt.Net.Core.Model.RawQuery("{}"), new List<string> { CommonConst.CommonField.KEY });
